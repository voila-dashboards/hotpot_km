...
class TestUpdatePooled(AsyncTestCase):
    ...
    async def test_path(self):
        with TemporaryDirectory() as tmp_dir:
            ...
            c.PooledKernelManager.kernel_pools = {NATIVE_KERNEL_NAME: 1}
            c.PooledKernelManager.strict_pool_names = True
            # Need mapping manager for paths to work:
            ...
            try:
                kid = await km.start_kernel(path=str(foo_mod))
                kernel = km.get_kernel(kid)
                self.assertIsNotNone(kernel)
    ...
    @gen_test(timeout=20)
    async def test_cwd_and_path_matching(self):
        with TemporaryDirectory() as tmp_dir:
            c = Config()
            c.PooledKernelManager.kernel_pools = {NATIVE_KERNEL_NAME: 1}
            c.PooledKernelManager.strict_pool_names = True
            ...
            try:
                kid = await km.start_kernel(cwd=tmp_dir, path=str(foo_mod))
                kernel = km.get_kernel(kid)
                self.assertIsNotNone(kernel)
