...
import asyncio
from traitlets import Unicode, Bool, Dict, List, Int, Enum, Float
...
class PoolMappingKernelManager(AsyncMappingKernelManager):
    ...
    fill_delay = Float(1, help="Wait time before filling the pool").tag(config=True)
    ...
    wait_at_startup = Bool(True).tag(config=True)
    ...
    def __init__(self, **kwargs):
        ...
        if self.wait_at_startup:
            ioloop.run_sync(lambda: self.fill_pool(delay=0.0))

    async def fill_pool(self, delay=None):
        tasks = []
        delay = delay if delay is not None else self.fill_delay
        # if we directly start filling the kernel pool, it may hurt performance for notebooks
        # that are just executing
    ...
    async def start_kernel(self, kernel_name=None, **kwargs):
        ...
        if kernel_name in self.pool_names and self._kernel_pools[kernel_name]:
            ...
            asyncio.create_task(self.fill_pool())
        else:
    ...
    async def _initialize(self, kernel_name, kernel_id_future):
        ...
        client.stop_channels()
        self.log.warning("Initialized kernel: %s", kernel_name)
        return kernel_id
